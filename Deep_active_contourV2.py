...
class Isoline_to_features(Module):
    ...
    def forward(self,
        ...
        isolines = mask * torch.exp(-(self.isolines[:,None,None] - distance)**(2)/(self.vars[:,None,None]))
        
        isolines2 = isolines
        x = np.squeeze(isolines2.cpu().detach().numpy())

        isolines0 = F.interpolate(isolines2, size=(self.shapes['0'][2],self.shapes['0'][3]), mode = 'bilinear')
        isolines1 = F.interpolate(isolines2, size=(self.shapes['1'][2],self.shapes['1'][3]), mode = 'bilinear')
        isolines2 = F.interpolate(isolines2, size=(self.shapes['2'][2],self.shapes['2'][3]), mode = 'bilinear')
        isolines3 = F.interpolate(isolines2, size=(self.shapes['3'][2],self.shapes['3'][3]), mode = 'bilinear')
        isolines4 = F.interpolate(isolines2, size=(self.shapes['4'][2],self.shapes['4'][3]), mode = 'bilinear')
        
        if compute_features_mask:

            mask2 = mask
            mask0 = F.interpolate(mask2, size=(self.shapes['0'][2],self.shapes['0'][3]), mode = 'bilinear')
            mask1 = F.interpolate(mask2, size=(self.shapes['1'][2],self.shapes['1'][3]), mode = 'bilinear')
            mask3 = F.interpolate(mask2, size=(self.shapes['3'][2],self.shapes['3'][3]), mode = 'bilinear')
            mask4 = F.interpolate(mask2, size=(self.shapes['4'][2],self.shapes['4'][3]), mode = 'bilinear')
        ...
        for i in range(5):
            ...
            f_s_i = (activations[str(i)] * ind_i).sum(dim = [2,3]) / ind_i.sum(dim = [1,2,3])[:,None]
            features.append(f_s_i)
            if compute_features_mask:
                features_mask.append(torch.sum(activations[str(i)] * masks[i],dim=(0,2,3))/torch.sum(masks[i],(0,2,3)))
...
class DAC():
    def __init__(self,
                 ...
                 model = multiscale,
                 dim = 512,
                 isolines = np.array([0.1,0.2, 0.4,0.6,0.8,1.]),
                 sigma = 7,
                 learning_rate = 5e-2,
                 clip = 1e-1,
        ...
        self.vars = torch.tensor(self.mean_to_var(self.isolines), device = 'cuda',dtype=torch.float32)
        ...
        self.normalizer.HERef = np.array([[0.47262014, 0.17700575],
    ...
    def convolve(self, 
        ...
        return torch.squeeze(F.conv1d(out, self.kernel[None,None,:],padding='same'))[:,margin:-margin]
    ...
    def mean_to_var(self,
                    isolines):

        mat = cdist(isolines[:,None],isolines[:,None])**2
        masked_a =  - np.min(np.ma.masked_equal(mat, 0.0, copy=False),0)/(8*np.log(0.5))
        return masked_a    
    ...
    def contour_to_distance_map(self,
                               contour):   
    ...
    def fit(self,
        ...
        img = img/255
        HE = np.clip(HE.reshape(2,img.shape[0], img.shape[1])[0],-clip_value,clip_value)/(2*clip_value)
        ...
        with torch.no_grad():
            ...
            if augment == True:
                ...
                for i in tqdm(range(self.nb_augment)):
                    ...
                    tensor_mask_anchor = torch.tensor(np.transpose(mask_anchor,(-1,0,1)), device = 'cuda')[None]
                    tensor_anchor = torch.tensor(np.transpose(img_anchor, (-1,0,1)), device = 'cuda')[None]
                    input_tensor = tensor_anchor
                    x = self.model(preprocess(input_tensor))

                    tensor_distance_anchor = torch.tensor(np.transpose(distance_anchor,(-1,0,1)), device = 'cuda')[None]
                    tensor_distance_anchor = F.interpolate(tensor_distance_anchor, size=(self.shapes['2'][2],self.shapes['2'][3]), mode = 'bilinear')
                    tensor_mask_anchor = F.interpolate(tensor_mask_anchor, size=(self.shapes['2'][2],self.shapes['2'][3]), mode = 'bilinear')
                    self.itf = Isoline_to_features(self.shapes,self.isolines, self.vars)

                    if i == 0:

                        features_anchor, self.features_anchor_mask = self.itf(self.activations,
                                                                              tensor_distance_anchor,
                                                                              tensor_mask_anchor)
                    else: 

                        tmp, tmp_mask = self.itf(self.activations,
                                                tensor_distance_anchor,
                                                tensor_mask_anchor)
                ...
                self.features_anchor_mask = [u/self.nb_augment for u in self.features_anchor_mask]
    ...
    def forward_on_epoch(self,
        ...
        features, self.features_mask = self.itf(self.activations,
                                                dst_map,
                                                mask,
        ...
        arr0 = torch.tensor([1.,1/2, 1/4.,1/8.,1/16.], device = 'cuda',dtype=torch.float32)
    ...
    def predict(self,
        ...
        scale = (torch.tensor([512.,512.], device='cuda',dtype=torch.float32)/torch.tensor(self.dims.copy(), device='cuda',dtype=torch.float32))
        stop = False
        contour0 = contour0 / np.flip(self.dims)
        contour0 = self.interpolate(contour0,
                                   self.nb_points)
        contour0 = np.roll(contour0,
                        axis = 1,
                        shift = 1)
        ...
        x = self.model(preprocess(input_tensor))
        self.mesh = torch.unsqueeze(torch.stack(torch.meshgrid(torch.arange(self.shapes['2'][2]),torch.arange(self.shapes['2'][3])),dim = -1).reshape(-1,2),dim=1)
        self.mesh = self.mesh.to(torch.float32).cuda() /torch.tensor(self.shapes['2'][2:],dtype=torch.float32,device = 'cuda')
        self.itf = Isoline_to_features(self.shapes,
                                       self.isolines,
                                       self.vars)
        energies = np.zeros((self.n_epochs,len(self.shapes),len(self.isolines)))
            
        contour = torch.from_numpy(contour0.astype(np.float32)).cuda()
        ...
        for i in range(self.n_epochs):
                ...
                if stop == False:
                    with torch.no_grad():    
                        ...
                        contour = contour -  scale * self.learning_rate * (self.ed**i) * gradient_direction
                        contour = contour.cpu().detach().numpy()
                        interpolated_contour = self.interpolate(contour, n = self.nb_points).astype(np.float32)
        ...
        dst_map, mask = self.contour_to_distance_map(contour)
        _, self.features_mask = self.itf(self.activations,
                                         dst_map,
                                         mask)
        ...
        for i in range(len(self.shapes)):
            score[i]= cos(self.features_anchor_mask[i], self.features_mask[i]).cpu().detach().numpy()
        ...
        score_correlogram = np.mean(np.sum(np.minimum(self.correlogram, self.correlogram_anchor), axis = -1)/ np.sum(np.maximum(self.correlogram,self.correlogram_anchor), axis = -1))
